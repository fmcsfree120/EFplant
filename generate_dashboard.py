...
// 圖例採「點選集合」模式：只保留已點過的線，圖表空白處可一鍵恢復全部。
function setupLegendSelectionMode(chart, seriesKeys) {{
  if (!chart || !chart.getZr) return;
  var selectedKeys = new Set();
  var zr = chart.getZr();

  chart.off('legendselectchanged');
  if (chart.__legendBlankClickHandler) zr.off('click', chart.__legendBlankClickHandler);

  function applySelection() {{
    var selected = {{}};
    seriesKeys.forEach(function(key) {{ selected[key] = selectedKeys.has(key); }});
    chart.setOption({{ legend: {{ selected: selected }} }});
  }}

  chart.on('legendselectchanged', function(params) {{
    selectedKeys.add(params.name);
    applySelection();
  }});

  chart.__legendBlankClickHandler = function(event) {{
    if (event.target) return;
    selectedKeys.clear();
    chart.setOption({{ legend: {{ selected: seriesKeys.reduce(function(all, key) {{ all[key] = true; return all; }}, {{}}) }} }});
  }};
  zr.on('click', chart.__legendBlankClickHandler);
}}
...
function _applySingleChart(chart, metricKey) {{
  ...
  setupLegendSelectionMode(chart, keys);
}}
...
"""
    ...
    # Login auditing is disabled. Do not embed the internal host address in
    # either the encrypted payload or the public static shell.
    full_html_with_ip = full_html.replace('"http://{LOCAL_IP}:47313"', "null")
    ...
    custom_index_html = STATIC_INDEX_HTML
    with open(output_path, "w", encoding="utf-8") as f:
